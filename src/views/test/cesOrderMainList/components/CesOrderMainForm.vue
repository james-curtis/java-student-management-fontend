  ...
  <div>
    ...
    <a-tabs v-model:activeKey="activeKey" animated @change="handleChangeTabs">
      ...
      <a-tab-pane tab="订单客户" key="cesOrderCustomer" :forceRender="true">
        <CesOrderCustomerForm ref="cesOrderCustomerForm" :disabled="formDisabled" />
      </a-tab-pane>
    </a-tabs>
  ...
  export default defineComponent({
    ...
    setup(props) {
      ...
      function classifyIntoFormData(allValues) {
        const main = Object.assign({}, allValues.formValue);
        return {
          ...main, // 展开
      ...
      function validateSubForm(allValues) {
        return new Promise((resolve, _reject) => {
          Promise.all([cesOrderCustomerForm.value.validateForm(1)])
            .then(() => {
              resolve(allValues);
            })
            .catch((e) => {
      ...
      const queryByIdUrl = '/test/cesOrderMain/queryById';
